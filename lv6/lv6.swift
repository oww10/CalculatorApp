...
class lv6ViewController: UIViewController, makeLineButtons{
    ...
    @objc func buttonTapped(_ sender: UIButton){
        // setTitle을 가져오기
        let buttonNum = sender.titleLabel?.text
        // 누른 버튼 타이틀 저장
        //resultText에 setTitle +
        let plusTitle = resultText.text! + buttonNum!
        //정수형으로 만들어서 맨 앞자리 0 제거
        let zeroDelNum = Int(plusTitle)!
        
        resultText.text = String(zeroDelNum)
